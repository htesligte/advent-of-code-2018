...
int main()
{
    ...
    for (i = 0; i < MAX_GUARD; i++) {
        if (guards[i] != NULL) {
            ...
            if (sleepiestGuard == NULL ||
                 sleepiestGuard->sleepingMinutes[sleepiestGuard->mostAsleepMinute] < guards[i]->sleepingMinutes[guards[i]->mostAsleepMinute]) {
    ...
    printf("At minute: %d\r\n", sleepiestGuard->mostAsleepMinute);
...
void fillSleepingMinutes(struct Guard* guard)
{
    ...
    // now loop over all the minutes and figure out at which minute the guy was most asleep
    for (i = 0; i < 60; i++) {
        if (guard->sleepingMinutes[i] > guard->sleepingMinutes[guard->mostAsleepMinute]) {
            guard->mostAsleepMinute = i;
        }
    }
}