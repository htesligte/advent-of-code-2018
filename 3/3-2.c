...
int main()
{
    ...
    // after this, we loop again to see if every position of this claim is free
    for (i = 0; i < NUM_LINES; i++) {
        claimIsNotOverlapping = true;
        ...
        for (y = 0; y < lineItems[i]->h; y++) {
            for (x = 0; x < lineItems[i]->w; x++) {
                pos = startPos + ((y * MAX_WIDTH) + x);
                if (positions[pos] != 1) {
                    claimIsNotOverlapping = false;
                    goto CONTINUE_OUTER_LOOP;
                }
            }
        }
        CONTINUE_OUTER_LOOP:
        if (claimIsNotOverlapping) {
            printf("Not overlapping: %d\r\n", lineItems[i]->i);
            break;
        }
    }
